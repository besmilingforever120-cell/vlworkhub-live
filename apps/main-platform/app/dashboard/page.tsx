...
export default async function DashboardPage() {
  ...
  const canOpenAdmin = user.platformRole === "SUPER_ADMIN" || user.platformRole === "IT_ADMIN";
  const adminHref = user.platformRole === "IT_ADMIN" ? "/admin/users" : "/admin";
  const dashboardCards = canOpenAdmin
    ? [...visibleApps, { appKey: "ADMIN", name: "Admin", description: "Create users, update status, and assign app access.", href: adminHref }]
    : visibleApps;
      ...
      <div className="rounded-[2rem] border border-white/10 bg-white/5 p-8">
        <div className="flex items-start justify-between gap-6">
          <div>
            ...
            {canOpenAdmin ? <Link href={adminHref} className="mt-6 inline-flex rounded-2xl bg-cyan-400 px-5 py-3 font-medium text-slate-950">Open Admin</Link> : null}
          </div>
          <LogoutButton />
        </div>
      </div>
