...
export function TrainingDetailView({ trainingId }: Props) {
  ...
  return (
    <div className="w-full flex flex-col">
      ...
      <section className="w-full px-4 pt-6 lg:px-6 xl:px-8"><div className="legacy-panel w-full max-w-none"><div className="legacy-panel-header"><div><h2>Quiz</h2><p>Embedded Microsoft Forms quiz</p></div></div><div className="legacy-panel-body"><div className="w-full h-[90vh] bg-slate-100 rounded-xl overflow-hidden">{String(training.quiz_iframe_link ?? "").trim() ? <div className="h-full w-full [&_iframe]:h-full [&_iframe]:w-full [&_iframe]:border-0" dangerouslySetInnerHTML={{ __html: String(training.quiz_iframe_link ?? "") }} /> : <div className="legacy-empty">No quiz iframe link configured.</div>}</div></div></div></section>
